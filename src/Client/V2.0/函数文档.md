...
该对象用于 socket 通信，是主要的类，其文件为**myWiznet5k_socket.py**。**要想使用该类，需要提前建立好 mySpi 类以及 myWinznet5k 对象，并在 mySocket 构造的时候传入对象**
...
构造一个 socket 对象，可用于执行 socket 等大多数通信。**使用过程中个，需要传入提前定义好的 spi 对象以及 myWinznet5k 对象才能正常使用，并且使用过程中需要初始化 socket_Buff 缓冲区**，具体初始化配置可以参考 [socketBuf 缓冲区设置](#2.11 socketBuf_init)
...
- 函数原型
  ...
  ```python
  __init__(self, spi_port, wiznet5k, local_port=5000)
  ```

  - 用于构造一个 socket 类
  - 参数：
    - `spi_port`: 传递 `mySpi` 对象，具体可参考 [mySpi](#mySpi 类)
    - `wiznet5k`: 传递 `myWinznet5k` 对象，具体可参考 [myWinznet5k](#myWinznet5k 类)
